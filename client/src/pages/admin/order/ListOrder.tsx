...
const ListOrder = () => {
  ...
  const handleOpentt = async (item: any) => {
    const order = await getOrder(item._id);
    // console.log(order)
    if (order?.data) {
      setModalInfo(order?.data);
...

    {
      ...
      render: (value, record, index) => (
        <Space size="middle" className="text-lg">
          ...
          {record?.products[0]?.status == "Yêu Cầu Hủy Đơn Hàng" || record?.products[0]?.status == "Xác Nhận Hủy Đơn Hàng" || record?.products[0]?.status == "Từ Chối Hủy Đơn Hàng" ?
            <button aria-label='delete'
              onClick={(event) => {
                event.stopPropagation();
                handleOpentt(record);
              }}
              type="button" className=" text-base text-red-500">
  ...
  return (
    <>
      ...
      <Modal
        ...
        onClose={handleClosett}
        aria-labelledby="modal-modal-title"
        aria-describedby="modal-modal-description"
        className="flex flex-col mt-52 items-center "
      >
          <div className="w-[500px] py-10 bg-gray-200 text-gray-900  shadow-2xl h-auto">
            <h3  className="font-bold  text-xl mb-2 ">Lý do </h3>
            <p>nội dung : {modalInfo?.products[0]?.reason}</p>
          </div>
      </Modal>
    </>
  );
